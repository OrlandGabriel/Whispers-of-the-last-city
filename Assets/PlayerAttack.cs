...
public class PlayerAttack : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (attackInput)
        {
            ...
            // Prevent attacking if not in combat
            if (!isInCombatStance)
            {
                if (showDebugLogs) Debug.Log("Cannot attack - not in combat stance!");
                return;
            }

            // Interaction check (prioritize interaction over attack)
            if (TryInteract())
            {
    ...
    void PerformAttack()
    {
        ...
        foreach (Collider col in hitMonsters)
        {
            ...
            if (monster != null && !monster.IsMonsterDead())
            {
                ...
                {
                    Vector3 effectPos = monster.transform.position + Vector3.up;
                    Instantiate(attackEffect, effectPos, Quaternion.identity);
                }
            }
            else
            {
                Debug.Log($"⚠ No valid MonsterAIfight found on {col.name}");
            }
        }

        // Fallback if no monster hit
        if (hitMonsters.Length == 0 && nearestMonster != null)
        {
            ...
            if (distance <= attackRange)
            {
                ...
                Debug.Log($"<color=green>✓ Fallback hit {nearestMonster.name} for {attackDamage}</color>");
                if (attackEffect != null)
                {
                    Vector3 effectPos = nearestMonster.transform.position + Vector3.up;
                    Instantiate(attackEffect, effectPos, Quaternion.identity);
                }
            }
            else
            {
                Debug.Log($"<color=red>✗ Nearest monster too far ({distance:F2}m)</color>");
            }
        }
